...
impl Boot for User{ type Req = bool; }
...
async fn save_fill() {
    ...
    let user=User{
     ...
     User::new_model(&db,None).fill(user).create(None).await.unwrap();
}
...
async fn find_one() {
    ...
    let founded = user_model.r#where(doc! {"name":"Smko"}).visible(vec!["password"]).first(None).await.unwrap();
...
async fn update() {
    let db = get_db().await;
    let user_model = User::new_model(&db,None);
    user_model.r#where(doc! {"name":"Smko"}).update(doc! {"age":3},None).await.unwrap();
    let user_model = User::new_model(&db,None);
    user_model.r#where(doc! {"name":"Smko"}).update(doc! {"$inc":{"age":1}},None).await.unwrap();
}
#[tokio::test]
async fn delete() {
    let db = get_db().await;
    let user_model = User::new_model(&db,None);
    user_model.r#where(doc! {"name":"Smko"}).delete(None).await.unwrap();
}
...
async fn transaction_with_session() {
    ...
    let user_model = User::new_model(&db, None);
    let user = user_model.r#where(doc! {"name": "TransactionUser"})
    ...
    let user_model = User::new_model(&db, None);
    user_model.r#where(doc! {"name": "TransactionUser"})
        .delete(None)
        .await
        .unwrap();
}
...
async fn get_db() -> Arc<Database> {
    Arc::new(Client::with_uri_str("mongodb://localhost:27017").await.expect("failed to connect")
        .database("test"))
}
